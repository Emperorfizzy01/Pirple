...
// Perform the check,send the originalCheckData and the outcome of the check and the outcome of the check process,to the next step in the process
workers.performCheck = function(originalCheckData) {
   ...
   // Special logic for accomodating a check that have never been tested before
     ...
     _data.update('checks',newCheckData.id,newCheckData,function(err) {
         if(!err) {
           ...
           if(alertWarranted) {
            ...
            debug('Error trying to save update to one of the file');
         }
     })
   };
};
   ...
   _logs.list(false,function(err,logs) {
       if(!err && logs && logs.length > 0) {
          logs.forEach(function(logName) {
            // Compress the data to a differnt file
...
workers.init = function() {
    ...
    // Execute all the checks immediately
     workers.gatherAllChecks();
    // Call all the loops so the checks will execute later on
    workers.loop();
